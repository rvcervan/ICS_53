...
*/
//Pointer Holders. Pointers to free in case of ctrl+c

petr_header* msg_header_holder = NULL;
/*
...
void free_auction_node(node_t* auction);
void free_user_node(node_t* user_node);
void free_server_users();
void free_server_auctions();
// int intlen(int n);
...
void* Green_Thread(void* vargp) {
    ...
    free(vargp); // Done copying args into thread's local stack
    ...
    while (1) { 
        ...
        // *** ANCLOSED ***
            ...
            if (win_name == NULL) {  // No winner
                ...
                reply_msg = realloc(reply_msg, msg_buf_size);
                sprintf(reply_msg, "%d\r\n%s\r\n%d", auction_id, win_name, win_price);
            }

            // send rep to all watchers of this removed auction
            petr_header rep_header;
            rep_header.msg_len = strlen(reply_msg)+1; //Handles empty auction list by default
            ...
            // Store removed auction into its winner won list
            if (win_name != NULL) {
                User_t* winner = FindUserName(win_name);
                List_t* winner_won_list = winner->won_auctions_list;
                insertInOrder(winner_won_list, removed_auction);
            }
            
            char* creator_name = removed_auction->creator_username;
            if(strcmp(creator_name, "")==0){//
                free_auction_node(removed_auction_node);
            }
            else{
                // Store removed auction into its creator sold list
                ...
                else{   // handle auctions with no creator name from input init file.
                    //delete auction node;
                    free_auction_node(removed_auction_node);
                }
            }
        ...
        // *** ANLIST ***
            ...
            char* reply_msg = calloc(1,sizeof(char));
        ...
        // *** ANBID ***
            ...
            //If auction exists but the user is not watching, server responds with EANDENIED (how does this happen?)
            ...
            if(strcmp(requesting_user->username, cur_auction->creator_username)==0){
                printf("error: creator can't bid\n");
                rep_header.msg_len = 0;
                rep_header.msg_type = EANDENIED;
                wr_msg(requesting_user->fd, &rep_header, NULL);
                continue;
            ...
            int msg_buf_size = 1;
            if(bid >= cur_auction->bin_price && cur_auction->bin_price != 0){
            ...
            while(watchers != NULL){
                ...
                //sends ANUPDATE to user watching besides bidder
                rep_header.msg_len = strlen(reply_msg)+1;
        ...
        // *** USRBLNC ***
            ...
            node_t* cur_node = (requesting_user->won_auctions_list)->head;
            while(cur_node != NULL){
                Auction_t* cur_auction = cur_node->value;

                blnc -= cur_auction->highest_bid;

                cur_node = cur_node->next;
            }

            cur_node = (requesting_user->sold_auctions_list)->head;
            while(cur_node != NULL){
                Auction_t* cur_auction = cur_node->value;
            ...
            char number[20];
            sprintf(number, "%d", blnc);
            
            petr_header usrblnc_rep_header;
            usrblnc_rep_header.msg_len = strlen(number)+1;
...
User_t* CreateAccount(char* username, char* password, int fd) {
    ...
    won_auctions->comparator = AuctionIDComparator;
    ...
    sold_auctions->comparator = AuctionIDComparator;
...
void free_auction_node(node_t* auction_node){
    Auction_t* auction = auction_node->value;
    free(auction->item_name);

    deleteList(auction->watchers_list);
    free(auction->watchers_list);

    free(auction_node->value);
    free(auction_node);
    return;
}

void free_user_node(node_t* user_node) {
    node_t* cur_won_node = NULL;
    node_t* next_won_node = NULL;
    node_t* cur_sold_node = NULL;
    node_t* next_sold_node = NULL;

    User_t* cur_user = user_node->value;
    // If online, close the fd
    if (cur_user->isOnline) {
        close(cur_user->fd);
    }
    // (online or not):
    // free username
    free(cur_user->username);
    // free password
    //free(cur_user->password);
    // free node and node value of won_auctions_list
    cur_won_node = (cur_user->won_auctions_list)->head;
    while (cur_won_node != NULL) {
        next_won_node = cur_won_node->next;
        free_auction_node(cur_won_node->value);
        cur_won_node = next_won_node;
    }
    // free node and node value of sold _auctions_list
    cur_sold_node = (cur_user->sold_auctions_list)->head;
    while (cur_sold_node != NULL) {
        next_sold_node = cur_sold_node->next;
        free_auction_node(cur_sold_node->value);
        cur_sold_node = next_sold_node;
    }
    free(cur_user->won_auctions_list);
    free(cur_user->sold_auctions_list);
    // free cur_user (node value)
    free(cur_user);
    // free (node)
    free(user_node); 
}
...
void free_server_users() {
    ...
    // free the head node of Users while the head node is not NULL (online or not)
    cur_node = Users->head;
    while (cur_node != NULL) {
        next_node = cur_node->next;
        free_user_node(cur_node);
        cur_node = next_node;
    }
}
...
void free_server_auctions() {
    ...
    // free the head node of Auctions while the head node is not NULL
    cur_node = Auctions->head;
    while (cur_node != NULL) {
        next_node = cur_node->next;
        free_auction_node(cur_node);
        cur_node = next_node;
    }
}
